...
export const addCurrency = (payload) => async (dispatch, getState) => {
  ...
  const { balance = 0, createdAt } =
    currencies.find(({ currency }) => currency === payload_.currency) || {};

  // update existing currency balance
  dispatch(
    receiveAddCurrency({
      ...payload_,
      balance: Number(balance + payload_.balance),
      createdAt: createdAt || new Date().toISOString(),
    })
  );
  // update total balance
  await dispatch(updateTotalBalance(payload_));
  // create history record
  dispatch(createRecord({ record: getHistoryRecord(payload_) }));
};
